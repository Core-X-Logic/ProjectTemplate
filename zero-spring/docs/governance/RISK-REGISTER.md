...
| **R-37** | `audit.domain` ve `notification.domain` paketlerinin `package-info.java`'sı **yok** | Modulith bu iki persistence paketini hiç kısıtlamıyor. ArchUnit Rule 4 donmuş listesinde (12 kayıt) — ayrı bir cırcır, Dalga 4 kapsamı dışı |
| **R-38** | **Modulith, string ile çözülen kenarı göremiyor** | Ölçüldü: `@FilterDef` `identity.domain`'deyken `audit`/`notification` entity'lerine `@Filter` eklendi ve `ModularityTests` **geçti** — oysa o iki modülün `allowedDependencies`'i `identity` içermiyor. Yani modül beyanı yalan olabilir ve test bunu onaylar. Filtre tanımı `shared.domain`'e taşınarak düzeltildi, ama **sınıf açık**: aynı körlük başka string-tabanlı bağlarda (cache adları, ShedLock kilit adları, i18n anahtarları) tekrarlanabilir |
