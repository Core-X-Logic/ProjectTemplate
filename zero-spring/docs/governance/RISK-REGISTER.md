...
| ~~**R-44**~~ (2026-08-08) | ~~`users.password_reset_code` ve `email_confirmation_code` DB'de **düz metin** ve **süresiz**~~ — expiry kolonu yoktu; "Invalid or expired reset code" mesajındaki *expired* koşulu kodda **hiç gerçekleşmiyordu**. (Bulgu bu şablondan türetilmiş bir projede, davet token'ı tasarlanırken açıldı; düzeltme upstream'e — buraya — ilk kez bu dilimde girdi) | ✅ **KAPANDI (V14 + `AccountRecoveryCodes`).** Kod tek kullanımlık ve 32 bayt `SecureRandom` olduğundan çevrimiçi brute force zaten pratik değildi (`RateLimitFilter` da ucu kapsıyor); gerçek pencere bir DB dump'ının **canlı** kodu vermesiydi. Artık iki akış da davet token'ı deseninde: DB yalnız **SHA-256 hex + expiry** tutar (reset **1 saat**, confirmation **72 saat**), süre `Clock` üzerinden okuma anında türetilir (ayrı EXPIRED durumu/temizlik işi bilinçli yok — R-46 sınıfı), eski düz-metin kolonlar **dokunulmadan** bırakıldı (bekleyen kodlar TRUNCATE edilmez; yeni akış yalnız `*_hash` okur → NULL hash hiçbir girdiyle eşleşmez, **fail-closed**; kolonlar rolling-deploy penceresi kapandıktan sonra ayrı V ile düşülecek). Kanıt `PasswordPolicyIT`: DB zemininde saklanan değer postalanan kodun SHA-256'sı (ve legacy kolon NULL), `MutableClock` ile süre **gerçekten** işliyor — 1s+1dk sonra reset kodu 400 + parola değişmemiş, 72s+1dk sonra confirmation kodu 400; bilinmeyen/uydurma kod aynı mesajla reddediliyor (oracle yok) |
...
| ~~R-08~~ | ~~Hibernate @Filter findById/lazy-collection'a uygulanmıyor (ikincil savunma boşluğu)~~ | M | M | Orta | **Closed** (2026-08-08, RLS taban çizgisi) | Zemin artık veritabanında: `tenant_id` taşıyan 9 tablonun 6'sı RLS politikalı (`V12`/`V13`), 3'ü ADR-0019 ile bilinçli muaf; `findById`/native-SQL/`@Filter`-unutması sınıfı DB'de kapanır (GUC yoksa 0 satır — fail-closed). Kanıt: `AppDbIdentityIT`(3) + `GucTenantContextIT`(10) + `RlsIdentityIsolationIT`(8) + `RlsAuditNotificationIsolationIT`(11) + kapsam guard'ı `RlsCoverageIT`(4). Kalan artıklar ayrı kayıtlı: `R-45` (host-global okuma genişliği), `R-46` (@Component GUC yazmıyor), `R-47` (`tenant_id`'siz join tabloları) | RLS ✅ |
...
| R-23 | Düşük artıklar + release-hardening: `read-all` endpoint'i testsiz; user_notifications.tenant_id dekoratif (user_id izolasyonu); ~~vendor CSS media-query uyarısı~~; ~~CI "Node 20 deprecation" uyarıları~~ | L | L | Düşük | **Kısmi Closed** (release-hardening 2026-07-21, [run 29822177991](https://github.com/Core-X-Logic/ProjectTemplate/actions/runs/29822177991)) | ✅ **CSS media uyarısı KAPANDI**: `@media (max-width: var(--breakpoint-lg))` → `64rem` — custom property bir media-feature değeri olarak geçersizdi, kural ölüydü ve build uyarı basıyordu; artık build **0 warning**. ✅ **Node 20 CI uyarıları KAPANDI**: action major uplift (checkout v4→v7, setup-node v4→v7, setup-java v4→v5, upload-artifact v4→v7, download-artifact v4→v8; hepsi Node 24) — tam log'da **0** "Node 20 deprecated" satırı; upload@v7→download@v8 handoff `sha256:f429bcb…955992f` digest'iyle **birebir** doğrulandı, davranış değişmedi. **Açık kalan (F3):** `read-all` testi + tenant_id semantiği. **Upstream / bizden kaynaklı DEĞİL:** `setup-java` iç bağımlılıklarından gelen Node-içi `DEP0040 (punycode)` / `DEP0169 (url.parse)` DeprecationWarning'leri — üçüncü-parti action kodundan, susturulamaz/düzeltilemez, hedeflenen Node 20 runtime deprecation ile alakasız | F3 (kalan) |
...
| ~~F5-R10~~ | ~~**Tenant create admin bootstrap yok**~~ | H | M | ~~Yüksek~~ | ✅ **Closed (2026-07-20)** — [Issue #1](https://github.com/Core-X-Logic/ProjectTemplate/issues/1); öngörülen tasarımla birebir kapandı (tek transaction, event/listener, `tenancy` yaprak kaldı). Kanıt yukarıdaki Issue #1 satırında | `TenantBootstrapIT` (6) + canlı smoke + re-boot smoke | — |
...
| ~~PROD-R27~~ | ~~Dockerfile'ı hiçbir otomasyon build etmiyor; PROD-R20 sertleştirmeleri hiçbir kapıda doğrulanmıyor~~ | CI `docker-build` job'u | Orta | ✅ **Closed (2026-07-21).** Öngörülen sıralama gerçekleşti: baseline 8/8 yeşil olduktan sonra `docker-build` gate'i eklendi (release'e bağlı). İmaj build ediliyor + non-root/healthcheck/prod-profil/heap-tavanı `docker image inspect` ile assert ediliyor; lokal + CI (run 29831107658) yeşil. `release` placeholder'ı da parametrik, cloud-agnostic deploy scaffold'una çevrildi (dry-run plan + `DEPLOY_ENABLED` guard'lı no-op; vendor lock-in yok). **Kalan (klonlayan işi, kod değil):** imajı registry'ye push + gerçek deploy hedefini bağlamak — `SETUP-NEW-PROJECT.md §6` checklist'i |
...
| PROD-R44a | ~~PayTR get-token **hash/token formülü** yalnız kendi offline vektörümüzle test ediliyordu (kendine-tutarlılık, harici oracle yok)~~ | Düşük | ✅ **KAPANDI (2026-07-21) — PayTR'nin KENDİ aracıyla harici doğrulama.** `dev.paytr.com/servis-test-araclari/hash-hesaplama` (iFrame API sekmesi), `PayTRTokenRequestTest`'in pinlediği birebir vektörle sürüldü: `merchant_id=123456, user_ip=203.0.113.10, merchant_oid=ZP42TESTOID01, email=ops@example.com, payment_amount=999, user_basket=W1siUHJvIChNT05USExZKSIsIjkuOTkiLDFdXQ==, no/max_installment=0, currency=TL, test_mode=1, salt/key=test-*`. PayTR aracının çıktısı `G0IZ3V/qo38nReuI/yukiPXL0LAjzu/1WtEbFX7h7nQ=` — commit'li `checkoutToken(...)` beklentimizle **byte-birebir**. Yani HMAC dizilimi, salt-sonda konumu VE `user_basket` base64 serileştirmesi PayTR'ın beklediğiyle aynı. Hash artık canlı bir başarısızlık nedeni değil |
| PROD-R44b | **PayTR get-token'ın alıcı kimlik alanları (email, user_ip, ad/adres/telefon) YER TUTUCU + canlı ağ round-trip'i hiç koşmadı.** `CheckoutRequest` host-operated akışta alıcı kimliği modellemiyor; adaptör placeholder gönderiyor. PayTR **paylaşılan test kimlik bilgisi yayınlamıyor** (resmî Postman env `XXXXXX` placeholder; `test_mode=1` gerçek mağaza hesabına biniyor) → **kredensiyalsiz canlı çağrı imkânsız.** Gerçek alıcı verisi + canlı get-token/bildirim/aktivasyon ilk kez canlı smoke'ta görünür | Orta | **Open — operatör-bağımlı.** Kapanış yolu: PayTR mağaza hesabı (veya test-mode kredensiyali) → checkout DTO'suna alıcı alanları + `scripts/sandbox-smoke.sh paytr` uçtan uca PASS (runbook §3.10). Formül tarafı R44a ile kapandı; kalan yalnız transport + alıcı verisi |
...
| PROD-R47 | **İki canlı iyzico çağrısı (`createCheckoutSession`, `confirmBySessionQuery`) hiçbir otomatik testte koşmuyor** — PROD-R37 deseninin iyzico'ya uzantısı, ve burada DAHA kritik: retrieve artık mutabakat job'unun da tek doğruluk kaynağı. IT'ler her ikisini SPI dikişinde sahteyle değiştirir; imza doğrulaması GERÇEK kodla, offline vektörle test edilir. Gerçek anahtar/hesap/parametre hataları ilk kez canlıda görünür | Orta | **⏸️ DEFERRED BY DECISION (2026-07-21) — SONRAKİ FAZ. KAPALI DEĞİL.** İş kararı: bu faz PayTR-only canlı doğrulamayla ilerliyor; iyzico canlı/sandbox kimlik bilgileri şu an YOK. **Sahibi:** release owner / operatör. **Kapanış koşulu:** iyzico sandbox merchant kaydı (`sandbox-merchant.iyzipay.com`, `sandbox-` önekli anahtarlar) → `scripts/sandbox-smoke.sh iyzico` uçtan uca PASS. **Tetik:** kimlik bilgileri sağlandığında / sonraki faz. Kod tarafı hazır ve offline kanıtlı; **canlı kanıt olmadan kapatılmayacak.** PROD-R45/R46 aynı ertelemeye bağlıdır (retry tekilliği, callback metodu, buyer alanları — hepsi iyzico canlı smoke'ta ölçülür) |
...
## 2FA (TOTP + kurtarma kodları) dilimi riskleri — 2026-07-21 eklendi

Dilim: giriş akışına 2. faktör kapısı (`AuthService.login` şifre-OK ile `issueTokenPair` ARASINDA;
2FA açıkken TOKEN BASILMAZ, `two_factor_challenges`'a challenge yazılır), `/api/auth/two-factor/verify`
(anonim, denetim-muaf, abonelik-muaf; TOTP veya kurtarma kodu; **fail-closed + oracle yok** — her
başarısızlık aynı jenerik 401), self-service `/api/profile/two-factor/{setup,enable,disable,
recovery-codes/regenerate}` (isAuthenticated, self-only; disable/regenerate şifreyi yeniden doğrular).
Yeni: AES-256-GCM alan şifreleme (`config.FieldEncryptionService` + `FieldEncryptionProperties` +
`FieldEncryptionKeyValidator`, JwtSecret* deseninin ikizi), TOTP kütüphanesi `dev.samstevens.totp:1.7.1`
(ZXing transitive'i HARİÇ tutuldu — QR görüntüsü üretmiyoruz, yalnız otpauth URI). Migration
`V10__two_factor.sql` (append-only). Bypass mutasyonu kanıtlı: `login`'deki kapı kaldırılınca
`TwoFactorLoginIT.twoFactorEnabledLoginYieldsAChallengeAndNoTokens` KIRMIZI ("required second factor
... but was false").

| ID | Bulgu | Şiddet | Durum |
|---|---|---|---|
| PROD-R49 | **`FIELD_ENCRYPTION_KEY` artık bir DEPLOY SECRET'i.** TOTP secret'ları AES-256-GCM ile şifreli saklanıyor; anahtar `zero.crypto.field-key` (base64 32 byte), JWT secret gibi **default'suz**. Kaybı/rotasyonu ele alınmadı: anahtar kaybolursa hiçbir 2FA kullanıcısı doğrulanamaz (fail-closed → herkes kurtarma koduna düşer), anahtar değişirse mevcut ciphertext'ler ÇÖZÜLEMEZ hale gelir (rotasyon bir re-encrypt migration'ı ister — yok). Çözülmemiş `${FIELD_ENCRYPTION_KEY}` placeholder'ı boot'ta reddedilir (base64 değil) | Orta | **Open (kayıtlı).** Mitigasyon: `FieldEncryptionKeyValidator` boot'ta yokluğu/yanlış uzunluğu/prod'da commit'li dev-test anahtarını reddeder (unit: `FieldEncryptionKeyValidatorTest`). Kalıcı kapanış: anahtar rotasyon prosedürü (eski→yeni re-encrypt) + secret yönetimi (KMS/Vault) — RELEASE-RUNBOOK'a girmeli |
| PROD-R50 | **Challenge/kurtarma-kodu tabloları `tenant_id` TAŞIMIYOR — bilinçli sapma (tasarım "tenant_id bigint" diyordu).** ArchUnit Rule 2, `tenant_id`'li her entity'ye Hibernate `@Filter` dayatır; `HibernateTenantFilterAspect` filtreyi her `@Service` çağrısında `TenantContext`'ten açar. Anonim verify ucunda `TenantContext` OPSİYONEL `X-Tenant` header'ından gelir → filtre, header yoksa meşru bir tenant kullanıcısının challenge'ını GÖRÜNMEZ yapardı (mutlu yolda fail-closed). `refresh_tokens` — aynanı aldığımız artefakt — aynı nedenle `tenant_id` taşımaz. Tenant, `user_id`'den (User.tenant_id) OTORİTER çözülür; basılan token'ın tenant claim'i User'dan gelir | Düşük | **Mitigated (test + tasarım).** `TwoFactorTenantIsolationIT`: (a) tenant kullanıcısı verify→token, claim doğru tenant; (b) **X-Tenant başka tenant gönderilse bile** basılan token kullanıcının KENDİ tenant'ı (primary-key `findById` filtreyi baypaslar → challenge'a bağlı user id ile çözülür, header ile değil); yanlış tenant header'ıyla `me` 403. İz: `V10` yorumu + entity javadoc'ları. Tenant kullanıcısı için verify, tıpkı login gibi `X-Tenant` gerektirir (pre-auth, ADR-0003) |
| PROD-R51 | **Kurtarma-kodu UX'i asgari + SMS/e-posta 2FA ve QR-ile-giriş KAPSAM DIŞI.** Kodlar yalnız enable/regenerate'te BİR KEZ dönüyor (sunucu hash saklıyor, tekrar gösteremez); "indir/yazdır", "kalan kod sayısı", "az kod kaldı" uyarısı yok. TOTP tek 2. faktör kanalı; SMS/e-posta OTP, WebAuthn/passkey, ve giriş ekranındaki QR akışı yok | Düşük | **Open (kapsam dışı — sonraki faz).** Frontend geçişi kodların bir kez gösterimini + kaydetme akışını taşımalı. Backend API'si (setup/enable → codes-once, regenerate) temiz OpenAPI ile hazır; `gen:api` sonraki frontend dilimine bırakıldı |
| PROD-R52 | **Kimlikli-ama-yetkisiz kullanım-dışı: kurtarma kodu tükenmesi kullanıcıyı kilitleyebilir.** 2FA açık kullanıcı authenticator'ını KAYBEDER ve tüm kurtarma kodlarını tüketirse, self-service disable şifre + oturum ister (challenge'ı geçemez → oturum yok) → kendini kurtaramaz. Admin-tarafı "kullanıcının 2FA'sını sıfırla" ucu YOK (self-only bilinçli) | Düşük | **Open (kayıtlı — operatör telafisi).** Şimdilik: host/tenant admin, kullanıcının `two_factor_enabled=false` + `two_factor_secret=null` + kurtarma kodlarını DB'den temizleyerek (ya da ileride eklenecek admin ucu) kurtarır. Kalıcı: yetkilendirilmiş "2FA reset" ucu (Side.HOST/tenant-admin izinli + negatif yetki testi) — sonraki faz |

**Güvenlik incelemesi sonucu (stack-review, 2026-07-21) — iki gerçek açık BULUNDU ve KAPATILDI (commit öncesi):**
- **Finding 1 (HIGH) — birinci faktör başarısı kilitlenme sayacını sıfırlıyordu.** `AuthService.login` şifre-OK'ta `failedLoginAttempts=0`'ı 2FA kapısından ÖNCE yapıyordu; şifresi sızmış ama cihazı olmayan saldırgan her yeniden-girişte sayacı sıfırlayıp 6-hanelik kodu SINIRSIZ deneyebiliyordu (yalnız IP limiter kalıyordu). Düzeltme: sayaç sıfırlaması yalnız `!twoFactorEnabled` dalına taşındı — sadece `verifyTwoFactor` başarısı (gerçek tam-kimlik) temizler; kilit KONTROLÜ login başında kalır. Negatif kanıt: `TwoFactorLoginIT.firstFactorSuccessDoesNotResetTheLockoutCounterAcrossRelogins` (eski kodda KIRMIZI: "reach ACCOUNT_LOCKED ... but was false" — hiç kilitlenmiyor).
- **Finding 2 (MEDIUM) — verify consume/decrement TOCTOU (atomik değildi).** `revokeIfActive`'in yanı başında olmasına rağmen challenge attempts düşümü ve recovery-kod tüketimi check-then-act idi → paralel: attempts LOST UPDATE, recovery-kod ve challenge ÇİFTE-HARCAMA. Düzeltme: (i) `TwoFactorChallengeRepository.findByTokenHashForUpdate` (`@Lock(PESSIMISTIC_WRITE)`) verify'ı challenge başına serileştirir; (ii) recovery tüketimi guarded UPDATE (`consumeIfUnconsumed`, affected-rows==1 — `revokeIfActive` deseni). Negatif kanıt: `TwoFactorConcurrencyIT` (eski kodda KIRMIZI: recovery `[200,200]` beklenen 1; paralel attempts `attempts_remaining` beklenen 1 iken 4). `tenant_id` sapması (PROD-R50) incelemede SAĞLAM onaylandı — değişmedi.

## RLS taban çizgisi riskleri — 2026-08-08 eklendi

| ID | Bulgu | Kaynak/İz | Etki | Olasılık | Durum / Mitigasyon |
|---|---|---|---|---|---|
| ~~R-08~~ | ~~Kiracı izolasyonu yalnız uygulama katmanında~~ | — | — | — | ✅ **KAPANDI (2026-08-08).** Kimlik ayrımı `V11__app_role.sql` (`zero_app`, NOSUPERUSER NOBYPASSRLS; `AppDbCredentialsValidator` prod'da tek-kimlik/commit'li-şifre/çözülmemiş-placeholder hâllerini açılışta reddeder) + GUC katmanı (`HibernateTenantFilterAspect`: `set_config(..., is_local=true)`, çıkışta çağıranın bağlamı geri kurulur — materialized CTE ile) + politikalar `V12` (identity) / `V13` (audit-notification) + kapsam guard'ı `RlsCoverageIT` (`information_schema` keşfi + `FORCE` denetimi + ADR-0019 makine-okunur muafiyet eşitliği + ≥9 taban). Türetilmiş projede aynı yığın 15 mutasyonla kanıtlandı; bu depoda scoped IT seti + tam suite yeşil. Artıklar: `R-45`, `R-46`, `R-47` |
| R-45 | **Host-global satırlar (`tenant_id IS NULL`) KURULU her kiracı bağlamına OKUNABİLİR.** `V12` politikası `users`/`roles` `USING`'ine bilinçli bir kol koyuyor: meşru gerekçe `ImpersonationService.backToImpersonator()` — taklit edilen KİRACI bağlamında HOST aktörünü ve onun host rolünü okumak zorunda (bugün çalışmasının sebebi Hibernate `@Filter`'ın `findById`'ye uygulanmaması; RLS'in böyle bir muafiyeti yok). Sonuç: zemin, tavandan bir satır sınıfı GENİŞ — uygulama katmanında `tenantFilter` o satırları her filtreli sorgudan dışlıyor, RLS dışlamıyor. `users` için bu, host operatörlerinin kullanıcı adı/e-postası ve parola HASH'idir | `V12__rls_identity.sql` başlığı (KARAR bloğu); `RlsIdentityIsolationIT.hostGlobalRowsAreVisibleToHostAndToAnEstablishedTenantContext`; kaynak projede mutasyonla ölçüldü: kol çıkarılınca `ImpersonationIT` **401** | Orta | Orta | **Open (bilinçli, sınırlanmış).** Üç sınır: (1) kol `WITH CHECK`'te YOK — kiracı bağlamı host-global satır YAZAMAZ (`RlsIdentityIsolationIT.aTenantContextCannotWriteAHostGlobalRow`); (2) kol **kurulu** bir kiracı bağlamına bağlı (`nullif(app.current_tenant,'') IS NOT NULL`) — bağlamsız yol 0 satır görür; (3) `organization_units` bu kolu ALMADI (kanıt yok). Daraltmanın tek yolu aktör okumasını `security definer` yardımcıya taşımak — ADR-0018'in bilinçli reddettiği yüzey, kendi ADR'ini ister |
| R-46 | **Arka plan işleri GUC yazmıyor.** `HibernateTenantFilterAspect` pointcut'ı `within(@Service *)`; `@Component` işler (ör. `SubscriptionLifecycleProcessor`) repository'yi doğrudan okuyor. Bugün zararsız — yalnız ADR-0019 ile muaf `saas` tablolarına dokunuyorlar. Ama aynı desen politikalı bir tabloya dokunduğu an iş **0 satır görüp BAŞARIYLA döner**: ne hata, ne log, ne kırmızı test. Kural ilk kez gerçek oldu: `seed/DataSeeder` bir `@Component` ve `users`/`roles` yazıyor — V12 ile uygulama HİÇ ayağa kalkmıyordu (INSERT `WITH CHECK`'e çarpıyor). Çözüm noktasal: seeder kendi bağlamını açıkça yazıyor (`announceHostContextToDatabase`, `run()` ve `reconcileStaticRolePermissions()` girişinde). `SubscriptionNotificationBridge` (aynı desen) bugün güvende, çünkü çağıranı her zaman host bağlamındaki bir `@Service` — bu bir tesadüf ve ArchUnit kuralı yazılana kadar öyle kalacak | `HibernateTenantFilterAspect` pointcut; ADR-0019 kuralı | Yüksek (sessiz veri kaybı sınıfı) | Orta | **Open (kural yazılı).** Politikalı bir tabloya dokunan her yeni scheduled/`@Component` iş, ya `@Service` sınırından geçmeli ya da GUC'u kendisi kurmalı. Kalıcı kapanış: pointcut'ı genişletmek **ya da** politikalı tabloya `@Component`'ten erişimi yasaklayan bir ArchUnit kuralı |
| R-47 | **`role_permissions` politikasız** — `tenant_id` kolonu yok, dolayısıyla ADR-0019 anlamında muaf DEĞİL, **aday değil**. Ama `roles` politikalandıktan sonra asimetri görünür hâle geldi: görünmeyen bir rolün izin satırları `role_id` bilinerek ham SQL ile hâlâ okunabilir. Aynı soru `user_roles`, `user_organization_units` ve — V13 ile görünür hâle gelen — `entity_property_changes` için de geçerli (`entity_change_id` bilinerek görünmeyen bir change'in diff'i okunabilir) | `V12__rls_identity.sql` `roles` bölümü; `V13` başlığı | Düşük (erişim ebeveyn id'sini bilmeyi gerektirir ve o id kiracı bağlamında görünmez) | Düşük | **Open (kayıtlı).** Kapanış: bu tablolara `EXISTS (SELECT 1 FROM <ebeveyn> ...)` biçiminde politika — ebeveynin kendi politikası alt sorguyu zaten daraltır. Hepsi tek dilimde kapanmalı; `RlsCoverageIT` bunları `tenant_id`'siz oldukları için GÖRMEZ |

## P2''-A (yönetilebilir sağlayıcı kimlik bilgileri + failover) riskleri — 2026-08-08 eklendi

Karar ve gerekçeler ADR-0020; tablo V16 (`billing_provider_credentials`, host-global, RLS adayı
değil — `tenant_id` yok).

| ID | Bulgu | Kaynak/İz | Etki | Olasılık | Durum / Mitigasyon |
|---|---|---|---|---|---|
| R-50 | **Failover, checkout transaction'ı içinde ARDIŞIK canlı sağlayıcı çağrıları yapar.** Tek round-trip kabulü (ADR-0014 dilim notu) "aday sayısı × adapter transport tavanı"na genişledi; DB bağlantısı bu süre boyunca tutulur. PayTR adapter'ının `RestClient.create()`'i timeout KONFIGÜRE ETMEZ (JDK default'ları) — asılı bir connect, failover'ın kendisini de asar | `BillingCheckoutService.startCheckout` (transaction notu); ADR-0020 §Sonuçlar | Orta | Düşük–Orta | **Open (kayıtlı, sınırlanmış).** Sınırlar: aday sayısı ≤3; 2-ardışık-hata → 60 sn cool-down kötü sağlayıcıyı rotasyondan çıkarır (`BillingCheckoutCircuitBreaker`); 4xx hiç failover tetiklemez. Kalıcı kapanış: adapter'lara açık connect/read timeout + (istenirse) session yaratmayı transaction dışına alan outbox deseni — ikisi de ayrı dilim |
| R-51 | **Stripe checkout anahtarı construction'da bağlanır.** `StripeBillingProvider` instance-scoped `StripeClient`'ı kurarken `getSecretKey()`'i BİR KEZ okur; portal'dan yazılan Stripe `secretKey` checkout için restart'a kadar etkisiz (webhook secret/publishable key çağrı anında çözülür). PayTR/iyzico'da böyle bir sınır yok | `BillingStripeConfig` javadoc'u; ADR-0020 §Sonuçlar | Düşük (Stripe uykuda, ADR-0017; portal varsayılanı listelemez) | Düşük | **Open (belgeli).** Kaldırmak = client'ı çağrı-başına kurmak (adapter içi değişiklik) — bilinçli olarak bu dilime alınmadı |
| R-52 | **PROD-R49 kapsam genişlemesi:** `FIELD_ENCRYPTION_KEY` artık 2FA secret'larına ek olarak sağlayıcı kimlik bilgilerini de açar. Anahtar kaybında fark: TOTP secret'ları yeniden üretilemez, sağlayıcı bilgileri operatörde durur — satır silinip yeniden girilir (`DELETE /api/billing/providers/{id}/credentials` + PUT). Rotasyon hâlâ re-encrypt migration'ı ister (yok) | PROD-R49; `BillingCredentialsResolver` (decrypt hatası YÜKSEK sesle 500 — sessiz env fallback bilinçli reddedildi) | Orta | Düşük | **Open (PROD-R49'a bağlı).** PROD-R49 kapanışı (rotasyon prosedürü + KMS/Vault) bu tabloyu da kapsamalı; runbook maddesine bu tablo eklenmeli |
| R-53 | **Devre kesici node-başına bellek-içi.** Çok node'lu kurulumda her node cool-down'u kendi hatalarından öğrenir: N node × 2 deneme = sağlayıcı gerçekte N×2 kez denenmiş olabilir. Ayrıca kesici durumu deploy'da sıfırlanır | `BillingCheckoutCircuitBreaker` javadoc'u | Düşük (maliyet birkaç fazladan timeout; para/doğruluk etkilenmez — `payments.provider` daima gerçekten keseni taşır) | Orta | **Open (bilinçli).** Node-ötesi durum = Redis/DB — transaction'dan bağımsızlık gereksinimi orada da geçerli ve karmaşıklığı bugünkü yüzeye değmez. Çok-node ödeme hacmi büyürse yeniden değerlendir |
