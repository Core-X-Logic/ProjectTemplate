...
docker compose exec postgres psql -U postgres -d zero -c "select * from shedlock;"
```
...
1. `docker compose exec postgres pg_isready -U postgres -d zero` → DB ayakta mı? (ad değil servis — §3.8 notu)
