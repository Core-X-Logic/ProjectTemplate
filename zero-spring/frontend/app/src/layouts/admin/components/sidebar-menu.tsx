...
export function SidebarMenu() {
  ...
  // Resolve a menu title, which is stored as an i18n message id.
      ...
      return (
        <AccordionMenuSub key={index} value={item.path || `root-${index}`}>
          <AccordionMenuSubTrigger className="text-sm font-medium">
            ...
            <span data-slot="accordion-menu-title">{label(item.title)}</span>
          </AccordionMenuSubTrigger>
          <AccordionMenuSubContent
            type="single"
            collapsible
    ...
    if (item.children) {
      ...
      return (
        <AccordionMenuItem
          ...
          className="text-[13px]"
        >
          <Link to={item.path || '#'}>{label(item.title)}</Link>
        </AccordionMenuItem>
      );
    }
  };
  ...
  const buildMenuHeading = (item: MenuItem, index: number): JSX.Element => {
    return (
      <AccordionMenuLabel key={index}>{label(item.heading)}</AccordionMenuLabel>
    );
  };
